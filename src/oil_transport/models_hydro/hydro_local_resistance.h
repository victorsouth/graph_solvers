...
namespace oil_transport {
    ...
    public:
        ...
        virtual double calculate_flow(double pressure_in, double pressure_out) override {
            double S = pde_solvers::circle_area(parameters.diameter);
            // TODO: density_working нужна здесь
            double density = buffer_in.density_std.value;
            double dp = pressure_in - pressure_out;
            double v = fixed_solvers::ssqrt(2 * dp / (parameters.xi * density));
            double flow = v * S;
            return flow;
        }
        /// @brief Вычисляет производные расхода по давлениям для локального сопротивления
        /// @param pressure_in Давление на входе
        /// @param pressure_out Давление на выходе
        ...
        virtual std::array<double, 2> calculate_flow_derivative(double pressure_in, double pressure_out) override {
            double dp = pressure_in - pressure_out;
            double S = pde_solvers::circle_area(parameters.diameter);
            // TODO: Здесь использовать density_working
            double density = buffer_in.density_std.value;
            // Производная от Q = S * sqrt(2 * dp / (xi * rho)) по dp:
            // dQ/d(dp) = S * (1/2) * sqrt((xi * rho) / (2 * dp)) * (2/(xi*rho))
            //          = S / sqrt(2 * dp * (xi * rho))
            double denom = fixed_solvers::ssqrt(2 * density * parameters.xi * dp);
            if (denom == 0 || dp == 0) {
                return { 0.0, 0.0 };
            }
            double common = S / denom;
