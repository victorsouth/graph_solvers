...
public:
    ...
    /// @param numerical_result Указатель для записи отладочных результатов решателя
    /// @return Рассчитанные давления и расходы для перенумерованного графа
                                     ...
                                     fixed_solver_result_t<-1>* numerical_result)
    {
        ...
        if (numerical_result == nullptr) {
            numerical_result = &numerical_result_carrier;
        }

        flow_distribution_result_t flow_result = solver.solve(initial_pressures, numerical_result);
