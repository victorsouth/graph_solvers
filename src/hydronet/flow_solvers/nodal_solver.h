...
struct nodal_solver_settings_t {
    ...
    /// @brief Критерий остановки по норме невязок
    double residuals_norm_tolerance{ std::numeric_limits<double>::quiet_NaN() };
    ...
    static nodal_solver_settings_t default_values() {
        ...
        result.residuals_norm_tolerance = 1e-8;
        return result;
    }
};
...
public:
    ...
    virtual std::vector<Eigen::Triplet<double>> jacobian_sparse(const Eigen::VectorXd& reduced_pressure) override
    {
        ...
        for (std::size_t i = 0; i < edges2_count; ++i) {
            ...
            ///вычисление производных потока по давлениям на вхлде и выходе
            auto [d_in, d_out] = edge.model->calculate_flow_derivative(p_from, p_to);

            if (!std::isfinite(d_in) || !std::isfinite(d_out)) {
                throw std::runtime_error("Derivarive is nan");
            }

            d_in = ensure_epsilon_value(d_in);
            d_out = ensure_epsilon_value(d_out);
...
private:
    ...
    static double ensure_epsilon_value(double value, double epsilon = 1e-6) {
        if (value < epsilon)
            return fixed_solvers::pseudo_sgn(value) * std::max(std::abs(value), epsilon);
        else
            return value;
    }
...
private:
    ...
    /// @brief Формирует результат по сокращенному вектору
    flow_distribution_result_t build_result(const Eigen::VectorXd& reduced_pressure) const {
...
public:
    ...
    /// @return Сырой результат работы численного метода
    flow_distribution_result_t solve(const Eigen::VectorXd& initial_pressures, 
        fixed_solver_result_t<-1>* solver_result = nullptr, bool use_reduced_estimation = true)
    {
        fixed_solver_parameters_t<-1, 0, golden_section_search> params 
            = prepare_solver_parameters();

        bool has_solver_result = true;
        fixed_solver_result_t<-1> result_carrier;//объект для хранения результатов решения (динамический размер)
        if (solver_result == nullptr) {
            solver_result = &result_carrier;
            has_solver_result = false;
        }

        // По соглашению при отсутствии заданного начального распределния используется случаное распределение
        ...
        //Вызов метода Ньютона-Рафсона для решения системы
        fixed_newton_raphson<-1>::solve(*this, initial, params, solver_result, nullptr);
        if (solver_result->result_code != numerical_result_code_t::Converged) {
            if (has_solver_result) {
                return flow_distribution_result_t();
            }
            else {
                // Пользователь запросил результат по методу Ньютона, 
                // не сможет узнать, что расчет не сошелся
                throw std::runtime_error("Nodal solver not converged");
            }
        }
        else {
            return build_result(solver_result->argument);
        }
    }
