...
PDE_SOLVERS_BRANCHES="$(git ls-remote --heads https://gitlab-ci-token:${CI_PDE_SOLVERS_TOKEN}@git.ktk.gubkin.ru/solvers/pde_solvers.git | awk '{print $2}' | cut -d'/' -f3-)"

#Задаем стандартные ветки
DEFAULT_FIXED_SOLVERS_BRANCH="main"
DEFAULT_PDE_SOLVERS_BRANCH="main"


#Определяем GRAPH_SOLVERS_BRANCH (ветку в репозитории GRAPH_SOLVERS)
if [ "$CI_PIPELINE_SOURCE" == "merge_request_event" ]; then
	export GRAPH_SOLVERS_BRANCH="$CI_MERGE_REQUEST_SOURCE_BRANCH_NAME"
else
	export GRAPH_SOLVERS_BRANCH="$CI_COMMIT_BRANCH"
fi
...
defining_branch "$FIXED_SOLVERS_BRANCHES" "$GRAPH_SOLVERS_BRANCH" "FIXED_SOLVERS_BRANCH" "$DEFAULT_FIXED_SOLVERS_BRANCH"
# Определяем название ветки в репозитории pde_solvers и записываем его в файл $filename
defining_branch "$PDE_SOLVERS_BRANCHES" "$GRAPH_SOLVERS_BRANCH" "PDE_SOLVERS_BRANCH" "$DEFAULT_PDE_SOLVERS_BRANCH"
# Записываем название ветки в репозитории corelib в файл $filename
echo "GRAPH_SOLVERS_BRANCH=$GRAPH_SOLVERS_BRANCH" >> "$filename"
