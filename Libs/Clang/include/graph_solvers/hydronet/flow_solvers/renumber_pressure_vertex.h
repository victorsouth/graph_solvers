...
private:
    ...
    {       
        ...
        // Перенос диагностики
        result.numerical_result = nodal_solver_results.numerical_result;
        result.residuals_analysis = analysis_backward_renumber(nodal_solver_results.residuals_analysis);

        if (nodal_solver_results.numerical_result.result_code != numerical_result_code_t::Converged)
            // Если расчет не сошелся, расходы и давления не найдены
            return result;
    ...
    /// @brief Перенос истории аргументов из перенумерованного подграфа в подграф с исходной нумерацией
    fixed_solver_result_analysis_t<-1> analysis_backward_renumber(const fixed_solver_result_analysis_t<-1>& analysis_renumbered) const {
        const std::vector<std::size_t>& new_vertex_to_old = get_new_vertex_to_old();
        fixed_solver_result_analysis_t<-1> analysis_original;
        analysis_original.target_function = analysis_renumbered.target_function;
        analysis_original.steps = analysis_renumbered.steps;
        // Перенумеровка истории давлений в узлах
        restore_original_order(analysis_renumbered.argument_history, new_vertex_to_old,
            analysis_original.argument_history);
...
public:
    ...
    /// @return Давления и расходы на исходном графе, результат численного метода и диагностика в полях результата
    flow_distribution_result_t solve(const Eigen::VectorXd& initial_pressures)
    {
        try {
            ...
            flow_distribution_result_t renumbered_result = solver.solve(renumbered_initial_pressures);
