...
/// @brief Инициализирует эндогенные свойства в буферах моделей ТРАНСПОРТНОГО графа на основе усредненных значений из измерений
/// Для параметров без измерений берёт значения из default_values.
...
{
    using namespace transport_buffer_helpers;

    hydro_measurements_averager_t averager;
    for (const auto& m : measurements) {
        averager.add_measurement(m);
    }
    oil_transport::endohydro_values_t avg_values = averager.get_average_hydro_values(default_values);
    ...
    for (const auto& edge_binding : edges) {
        ...
        if (auto source_model = dynamic_cast<oil_transport::transport_source_model_t*>(model)) {
            ...
            init_single_sided_buffer_from_average(buffer, avg_values);
        }
            ...
            init_two_sided_buffer_from_average(buffers_for_getter, avg_values);
        }
        else {
            ...
            pipe_buffer_initializer_t<layer_type, oil_transport::endohydro_values_t>::init_density(
...
/// @brief Инициализирует эндогенные свойства в буферах моделей ГИДРАВЛИЧЕСКОГО графа на основе усредненных значений из измерений
/// Для параметров без измерений берёт значения из default_values.
/// @param layer_offset определяет, в какой слой буфера трубы записывать значения (текущий или предыдущий).
/// @param default_values Источник значений по умолчанию для параметров, измерения которых отсутствуют
template <typename PipeSolver, typename LumpedCalcParametersType = oil_transport::endohydro_values_t>
inline void init_endogenous_in_buffers(
    const oil_transport::hydraulic_graph_t& hydro_graph,
    const std::vector<graph_quantity_value_t>& measurements,
    graphlib::layer_offset_t layer_offset,
    const oil_transport::endohydro_values_t& default_values = oil_transport::endohydro_values_t{})
{
    ...
    auto edges = hydro_graph.get_edge_binding_list();
    for (const auto& edge_binding : edges) {
        oil_transport::hydraulic_model_t* model =
            hydro_graph.get_model_incidences(edge_binding).model;
        ...
        else if (auto pipe_model = dynamic_cast<oil_transport::hydraulic_pipe_model_t<PipeSolver>*>(model)) {
            using layer_type = typename PipeSolver::layer_type;
            using buffer_type = pde_solvers::ring_buffer_t<layer_type>;
            auto* pipe_buffer = reinterpret_cast<buffer_type*>(buffer_pointer);
...
private:
    ...
    graphlib::transport_problem_info<graph_quantity_value_t> step_flow_propagation(
        ...
        graphlib::structured_flow_distribution_result* full_graph_result)
    {
        // Транспортный шаг
        ...
        graphlib::structured_flow_distribution_result solver_result =
            hydro_solver.solve(graphlib::solver_estimation_type_t::FromPreviousLayer);
        if (full_graph_result != nullptr)
            *full_graph_result = solver_result;
        ...
        graphlib::structured_flow_distribution_result* full_graph_result)
    {
        ...
        bool use_flows_from_buffers = true;
        graphlib::transport_problem_info<graph_quantity_value_t> transport_result = 
            solver_transport.step(time_step, flow_measurements, endogenous_measurements, use_flows_from_buffers);
...
public:
    ...
    graphlib::transport_problem_info<graph_quantity_value_t> solve(
        ...
        graphlib::structured_flow_distribution_result* full_graph_result = nullptr)
    {
        auto [flow, endogenous] = classify_measurements(measurements);
        return solve(flow, endogenous, confidence_reset, full_graph_result);
    }
    ...
    {
        auto [flow, endogenous] = classify_measurements(measurements);
        return step(time_step, flow, endogenous, full_graph_result);
    }
        ...
        graphlib::structured_flow_distribution_result* full_graph_result = nullptr)
    {
        if (settings.solution_method == hydro_transport_solution_method::FindFlowFromMeasurements) {
            return step_flow_propagation(time_step, flow_measurements, endogenous_measurements, full_graph_result);
        }
        else {
            return step_flow_calculation(time_step, flow_measurements, endogenous_measurements, full_graph_result);
        }
    }
