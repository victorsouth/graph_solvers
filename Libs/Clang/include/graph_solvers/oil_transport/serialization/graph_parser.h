...
// TODO: перенсти в string_helpers fixed_solvers
/// @brief Переводит строку из ASCII символов в нижний регистр
...
// TODO: перенсти в string_helpers fixed_solvers
/// @brief Возвращает строку, представляющуюу собой входную ASCII строку, 
...
struct graph_json_data {
    ...
    std::unordered_map<std::string, pump_json_data> pumps;
    /// @brief Список свойств обратных клапанов с привязкой к имени
    std::unordered_map<std::string, check_valve_json_data> check_valves;
    ...
    static std::unordered_map<std::string, lumped_json_data> convert_to_lumped_data(
        ...
        auto converter_to_lumped = [](const std::pair<const std::string, JsonModelParameters>& kvp) {
            ...
            else if constexpr (std::is_same<JsonModelParameters, pump_json_data>::value) {
                converted.is_initially_opened = original_data.is_initially_started;
            }
            else if constexpr (std::is_same<JsonModelParameters, check_valve_json_data>::value) {
                if (original_data.is_initially_closed_static == fixed_solvers::nullable_bool_t::False) {
                    converted.is_initially_opened = true;
                }
                else if (original_data.is_initially_closed_static == fixed_solvers::nullable_bool_t::True) {
                    converted.is_initially_opened = false;
                }
                else { // Undefined
                    converted.is_initially_opened = true;
                }
...
public:
    ...
    > get_transport_object_parameters() const
    {
        ...
        // Насосы обрабатываем как сосредоточенные транспортные объекты
        convert_objects<transport_lumped_parameters_t>(convert_to_lumped_data(pumps), result);
        ...
        // Обратные клапаны обрабатываем как сосредоточенные транспортные объекты
        convert_objects<transport_lumped_parameters_t>(convert_to_lumped_data(check_valves), result);
    ...
    > get_hydro_object_parameters() const
    {
        ...
        convert_objects<oil_transport::iso_qsm_pump_parameters>(pumps, result);
        convert_objects<oil_transport::iso_qsm_check_valve_parameters>(check_valves, result);
        copy_and_convert_pipes(pipes);
...
protected:
    ...
    /// @brief Зачитка списка свойств насосов
    static std::unordered_map<std::string, pump_json_data> load_pump_data(const std::string& filename);
    /// @brief Зачитка списка свойств обратных клапанов
    static std::unordered_map<std::string, check_valve_json_data> load_check_valve_data(const std::string& filename);
...
public:
        ...
        const std::string& path, const std::string& tags_filename = "")
    {
        ...
        // TODO: из-за std::move в prepare_transport_siso_graph в hydro_transport_siso_graph нет параметров транспортных моделей. Нельзя создать hydro_transport_task из них
...
template <typename T>
inline constexpr auto get_field_names() {
