...
public:
    ...
    /// @brief Возвращает максимальный индекс вершины, встречающийся в ребрах графа
    std::size_t get_max_vertex_index() const
    {
        if (empty())
            throw std::runtime_error("Graph is empty, cannot count vertices");
        std::size_t max_vertex = 0;
        for (const auto& edge1 : edges1) {
            max_vertex = std::max(max_vertex, edge1.get_vertex_for_single_sided_edge());
        }
        for (const auto& edge2 : edges2) {
            max_vertex = std::max(max_vertex, edge2.from);
            max_vertex = std::max(max_vertex, edge2.to);
        }
        return max_vertex;
    }
        ...
        size_t max_edge_count = std::numeric_limits<size_t>::max()) const
    {
        ...
        // TODO: Здесь vertices - вектор. При этом формат данных topology.json позволяет давать вершинам произвольную НЕпоследовательную нумерацию. 
        // Вектор с таким не справится
        for (const auto& vertex : vertices) {
            size_t start_vertex = vertex.first;
            if (!visited_vertices[start_vertex]) {
                subgraph_t subgraph = _bfs_search_subgraph_routine(start_vertex, vertices,
